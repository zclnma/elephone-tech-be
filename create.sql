...
create table customer (id uuid not null, contact varchar(255), email varchar(255), name varchar(255), primary key (id))
create table device (id uuid not null, color varchar(255), imei varchar(255), name varchar(255), passcode varchar(255), primary key (id))
...
create table transaction (addition_info varchar(255), auth_signature TEXT, battery varchar(255), conf_signature TEXT, created_date timestamp, deposit varchar(255), finalised_time timestamp, is_deleted boolean, issue TEXT, last_modified_date timestamp, notification_method varchar(255), reference varchar(255), resolution varchar(255), status varchar(255), device_id uuid not null, created_by uuid, customer_id uuid not null, finalised_by uuid, init_store_id uuid, store_id uuid, primary key (device_id))
create table transaction_inspections (transaction_device_id uuid not null, inspections varchar(255))
create table transaction_products (transaction_device_id uuid not null, description varchar(255), number varchar(255), price varchar(255))
...
alter table if exists transaction add constraint UK_btane64ls5uag9j1yuq0yepow unique (customer_id)
...
alter table if exists transaction add constraint FKnbpjofb5abhjg5hiovi0t3k57 foreign key (customer_id) references customer
alter table if exists transaction add constraint FKto62t31xxfv2clk7v3ils9um6 foreign key (device_id) references device
...
alter table if exists transaction_inspections add constraint FK8yhwpqu0s9b6q42owvk8x90dr foreign key (transaction_device_id) references transaction
alter table if exists transaction_products add constraint FKj8gn4tn3oj8lqdec8qhyqlhqn foreign key (transaction_device_id) references transaction
...
create table move_path (id uuid not null, created_date timestamp, last_modified_date timestamp, store_id uuid not null, transaction_id uuid not null, move_path_order int4, primary key (id))
create table store (id uuid not null, abn varchar(255), address varchar(255), company_name varchar(255), contact varchar(255), created_date timestamp, deleted_at timestamp, email varchar(255), is_deleted boolean, last_modified_date timestamp, name varchar(255), postcode varchar(255), reference int4, sequence serial, state varchar(255), suburb varchar(255), warranty int4, primary key (id))
create table transaction (id uuid not null, addition_info varchar(255), auth_signature TEXT, battery varchar(255), conf_signature TEXT, created_date timestamp, deposit varchar(255), finalised_time timestamp, is_deleted boolean, issue TEXT, last_modified_date timestamp, notification_method varchar(255), reference varchar(255), resolution varchar(255), status varchar(255), created_by uuid, customer_transaction_id uuid, device_transaction_id uuid, finalised_by uuid, init_store_id uuid, store_id uuid, primary key (id))
...
alter table if exists move_path add constraint FK5nglfbfoohp6tfdpa51gm82h5 foreign key (transaction_id) references transaction
alter table if exists transaction add constraint FKrpx9b019itm9mwitcw53ui0tb foreign key (created_by) references employee
alter table if exists transaction add constraint FKahqmfu6awh4fdjodafpk6jh8l foreign key (customer_transaction_id) references customer
alter table if exists transaction add constraint FKnyg48ynyxgslnnudlrxpm9s40 foreign key (device_transaction_id) references device
alter table if exists transaction add constraint FKcckopmu8rb9im7f2a20jkeloh foreign key (finalised_by) references employee
alter table if exists transaction add constraint FKkwnf760xnl07l48nkelkaeip7 foreign key (init_store_id) references store
alter table if exists transaction add constraint FK1eprpvnt4sd424pavi0mc91a5 foreign key (store_id) references store
